...
bool measure_flow_slm(float * flow_rate)
{
    ...
    Wire.beginTransmission(SFM3300_I2C_ADDRESS);
    Wire.write(SFM3300_MEAS_CMD_BYTE_1);
    Wire.write(SFM3300_MEAS_CMD_BYTE_2);
    ...
    Wire.requestFrom(SFM3300_I2C_ADDRESS, 3);
    int32_t count = 0;
    while(count < 3)
    {
        if(Wire.available())
...
bool read_serial_number(uint32_t * sn)
{
    ...
    if(!i2c_write_registers(SFM3300_I2C_ADDRESS, 
        SFM3300_READ_SN_BYTE_1, &cmd, 1, 10))
    {
        success = false;
    }
    delay(2);
    ...
    uint8_t data[6];
    if(!i2c_read_registers(SFM3300_I2C_ADDRESS, 0, &data[0], 6, 10)) // TODO sends extra byte
    {
        success = false;
    }
